# Hosting: git-bootcamp.rgtlab.org
...
`https://git-bootcamp.rgtlab.org` resolve. The remaining
...
1. **Create the Netlify site.** In the Netlify dashboard, or
   ...
   ```bash
   netlify sites:create --name git-bootcamp-rgtlab
   ```
...
3. **Point the subdomain at Netlify.** `rgtlab.org` is not on
   ...
   ```text
   git-bootcamp   CNAME   <netlify-site>.netlify.app.
   ```

   Then add `git-bootcamp.rgtlab.org` as a custom domain on
   the Netlify site so Netlify provisions the TLS
   certificate.
...
```bash
curl -I https://git-bootcamp.rgtlab.org
```
